...
#path of the project
path_proj<-"D:/products/NORTH_FORECAST"
...
P2_PROXY_SECTION_SHARE<-function(sales_rev,start_date,end_date,df_store)
{
  ...
  print("this is sales revenue")
  print(sales_rev)
  sales_tag<-colnames(sales_rev)
  print("modified sales rev")
  setDT(sales_rev)
  print(sales_rev[,grep(start_date,colnames(sales_rev)):grep(end_date,colnames(sales_rev))])
...
rem_period<-format(seq.Date(as.Date(mt_user$REM_START_PERIOD),as.Date(mt_user$REM_END_PERIOD), by = "month"),
                   format = "%b-%Y")
ip_months <- ip_months[-which(ip_months %in% rem_period)]
...
#assembling the data at store-dept level
...
fwrite(nso_proxy_sales, file.path(path_proj, "PREPROCESSED_OPS","NSO", "ARTICLE_SALES", "NSO_STR_ART.csv"))
