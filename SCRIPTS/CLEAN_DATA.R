...
path_proj<- "D:/products/NORTH_FCST"
...
fcst_ip_months <- format(seq.Date(as.Date(user_ip$START_PERIOD),as.Date(user_ip$END_PERIOD), by = "month"),
                      format = "%b-%Y")
#these months need to be removed becoz of COVID impact
rem_months<- format(seq.Date(as.Date(user_ip$REM_START_PERIOD),as.Date(user_ip$REM_END_PERIOD), by = "month"),
                    format = "%b-%Y")
fcst_ip_months<- fcst_ip_months[-which(fcst_ip_months %in% rem_months)]
...
#v-mart stores separated based on the zone
vmart_stores_rev<- store_art_rev[which(store_art_rev$site_code %in% store_mstr[which(store_mstr$zone != "south"),"site_code"])]
vmart_stores_qty<- store_art_qty[which(store_art_qty$site_code %in% store_mstr[which(store_mstr$zone != "south"),"site_code"])]
...
rem_start_date <- as.Date(user_ip$REM_START_PERIOD)
rem_end_date <- as.Date(user_ip$REM_END_PERIOD)

##calculating the months by which store has opened
...
p1_stores<-store_mstr[which(store_mstr$month_diff>=24),]
p2_stores<-store_mstr[which(store_mstr$month_diff<24 & 
                              store_mstr$month_diff>=2 ),]
nso_stores<-store_mstr[which(store_mstr$month_diff<2),]

setDT(p1_stores)
setDT(p2_stores)
setDT(nso_stores)

p1_stores<- p1_stores[,-c("month_diff")]
p2_stores<- p2_stores[,-c("month_diff")]
nso_stores<- nso_stores[,-c("month_diff")]
all_stores<- rbind(p1_stores, p2_stores,nso_stores) 


# making adjustment as per the covid19 impact
# making the 2021 sales-values to 90 per_cent of 2019 sales-values
modified_cols_ind <- grep("2021", colnames(vmart_stores_rev))
cols_for_modif_ind <- grep("2019", colnames(vmart_stores_rev))

vmart_stores_rev[which(rowSums(vmart_stores_rev[, (cols_for_modif_ind[1]):
                                                  cols_for_modif_ind[length(cols_for_modif_ind)]]) != 0), (modified_cols_ind[1] + 1):modified_cols_ind[length(modified_cols_ind)]] <- 0.9 * vmart_stores_rev[which(rowSums(vmart_stores_rev[, (cols_for_modif_ind[1]):
                                                                                                                                                                                                                                              cols_for_modif_ind[length(cols_for_modif_ind)]]) != 0), (cols_for_modif_ind[1] + 3):
                                                                                                                                                                                                               cols_for_modif_ind[length(cols_for_modif_ind)]]
vmart_stores_qty[which(rowSums(vmart_stores_qty[, (cols_for_modif_ind[1]):
                                                  cols_for_modif_ind[length(cols_for_modif_ind)]]) != 0), (modified_cols_ind[1] + 1):modified_cols_ind[length(modified_cols_ind)]] <- round(0.9 * vmart_stores_qty[which(rowSums(vmart_stores_qty[, (cols_for_modif_ind[1]):
                                                                                                                                                                                                                                                    cols_for_modif_ind[length(cols_for_modif_ind)]]) != 0), (cols_for_modif_ind[1] + 3):
                                                                                                                                                                                                                     cols_for_modif_ind[length(cols_for_modif_ind)]])



#separate the p1, p2 and nso store revenue
p1_stores_rev<- vmart_stores_rev[which(vmart_stores_rev$site_code %in% p1_stores$site_code),]
p2_stores_rev<- vmart_stores_rev[which(vmart_stores_rev$site_code %in% p2_stores$site_code),]
nso_stores_rev<- vmart_stores_rev[which(vmart_stores_rev$site_code %in% nso_stores$site_code),]



p1_stores_qty<- vmart_stores_qty[which(vmart_stores_qty$site_code %in% p1_stores$site_code),]
p2_stores_qty<- vmart_stores_qty[which(vmart_stores_qty$site_code %in% p2_stores$site_code),]
nso_stores_qty<- vmart_stores_qty[which(vmart_stores_qty$site_code %in% nso_stores$site_code),]
...
p1_stores_rev <- p1_stores_rev[which(rowSums(p1_stores_rev[, (ncol(p1_stores_rev) - 47):(ncol(p1_stores_rev) - 1)]) != 0), ]
p1_stores_qty <- p1_stores_qty[which(rowSums(p1_stores_qty[, (ncol(p1_stores_qty) - 47):(ncol(p1_stores_qty) - 1)]) != 0), ]
p2_stores_rev <- p2_stores_rev[which(rowSums(p2_stores_rev[, (ncol(p2_stores_rev) - 47):(ncol(p2_stores_rev) - 1)]) != 0), ]
p2_stores_qty <- p2_stores_qty[which(rowSums(p2_stores_qty[, (ncol(p2_stores_qty) - 47):(ncol(p2_stores_qty) - 1)]) != 0), ]
