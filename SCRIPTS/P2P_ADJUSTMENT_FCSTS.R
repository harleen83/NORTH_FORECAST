...
complete_str_mstr<- fread("D:/products/FORECAST_FINAL_PROJECT/STORE_MSTR/STORE_MSTR.csv")
  
preprocessed_ip_path<- list.files(file.path(path_proj,"PREPROCESSED_OPS"),full.names=TRUE)
#preprocessed_ip_path<- "D:/products/FORECAST_FINAL_PROJECT - SOUTH/PREPROCESSED_OPS/NSO/DEPT_SALES/NSO_STR_DEPT_MOD1.csv"
hwa_fcst_path<- list.files(file.path(path_proj,"HWA_FCST_OPS"),full.names=TRUE)
#hwa_fcst_path<- "D:/products/FORECAST_FINAL_PROJECT - SOUTH/HWA_FCST_OPS/NSO/NSO_STR_DEPT/NSO_STR_DEPT_FCST1.csv" 
str_mstr_path<- list.files(file.path(path_proj, "COMPANY_LEVEL_SEPARATED_DATA"),full.names = TRUE)
#str_mstr_path<- "D:/products/FORECAST_FINAL_PROJECT - SOUTH/COMPANY_LEVEL_SEPARATED_DATA/NSO/NSO_STR_MSTR/NSO_STR_MSTR_V3.csv"  
...
#dt_str_dept_ip<- fread("D:/products/FORECAST_FINAL_PROJECT - SOUTH/PREPROCESSED_OPS/NSO/DEPT_SALES/NSO_STR_DEPT_MOD1.csv")
#reading the fcst file
dt_str_dept_fcst<- fread(str_hwa_fcst_path)
#dt_str_dept_fcst<- fread("D:/products/FORECAST_FINAL_PROJECT - SOUTH/HWA_FCST_OPS/NSO/NSO_STR_DEPT/NSO_STR_DEPT_FCST1.csv" )
#reading the store-mstr
dt_str_mstr<- fread(store_mstr_path)
#dt_str_mstr<- fread("D:/products/FORECAST_FINAL_PROJECT - SOUTH/COMPANY_LEVEL_SEPARATED_DATA/NSO/NSO_STR_MSTR/NSO_STR_MSTR_V3.csv")
...
remove_months <- GET_SEQUENCE_OF_DATES(mt_user$REM_START_PERIOD, mt_user$REM_END_PERIOD)
ip_months <- ip_months[-which(ip_months %in% remove_months)]
...
#removing the months with COVID-impact from the weighted_months
weighted_months <- weighted_months[-which(weighted_months %in% remove_months)]
...
#pujo_coeff[,2:ncol(pujo_coeff)]<- as.double(pujo_coeff[,2:ncol(pujo_coeff)])
...
pujo_strs_fcst[is.na(pujo_strs_fcst$p2p_adjusted_sales)]<-0
dc_strs_fcst[is.na(dc_strs_fcst$p2p_adjusted_sales)]<-0
pdc_strs_fcst[is.na(pdc_strs_fcst$p2p_adjusted_sales)]<-0
...
#looping from 2 to the length of forecast months
...
write_xlsx(dt_str_dept_adj_fcst, output_path)
