...
#path_output<-"D:/products/forecast_all_modules"
...
rem_period<-format(seq.Date(as.Date(user_input$REM_START_PERIOD),as.Date(user_input$REM_END_PERIOD), by = "month"),
                   format = "%b-%Y")
...
for(yr_files in yr_folders)
{
  ...
  df_lost['lost_sale_status'][which(df_lost$`fill_rate`<0.7),]<-"Lost sale"
  ...
  df_lost['lost_sale_qty'][which(df_lost$fill_rate<0.7),]<-(1-df_lost['fill_rate'][which(df_lost$`fill_rate`<0.7),])*0.5*(df_lost['Month_1'][which(df_lost$`fill_rate`<0.7),])/4
